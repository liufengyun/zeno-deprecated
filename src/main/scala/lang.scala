...
object lang {
  ...
  class StdLib(val c: Circuit) {
    ...
    def (t1: Exp[S]) ~[S, T](t2: Exp[T]): Exp[S ~ T] = pair(t1, t2)
    ...
    def mux(cond: Exp[Bit], x: Exp[Bit], y: Exp[Bit]): Exp[Bit] = (cond.not || x) && (cond || y)
